  ...
  return (
    <motion.div
      ...
      onDragEnd={(event, info) => {
        ...
        const velocityMagnitude = Math.sqrt(currentVelocityX ** 2 + currentVelocityY ** 2);
      ...
      style={{ ...style, rotateX, rotateY, opacity, willChange: "transform" }}
      animate={controls}
      whileHover={{ scale: 1.02 }}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      className={`draggable-card-body ${className}`}
    >
      {children}
      <motion.div style={{ opacity: glareOpacity }} className="draggable-card-glare" />
    </motion.div>
  );
};
